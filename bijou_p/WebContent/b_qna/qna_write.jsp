...
<script type="text/javascript">
  ...
  function fn_pw(obj) {
	  var pw = document.getElementById("qna_pw").value;
  ...
  function fn_pw_p(obj) {
	  ...
	  if(pw == "" || pw == null){
		  ...
		  obj.action = "${contextPath}/qna/write_product.do";
		  obj.submit();
	  }
...
<!--Main Header-->
	...
	<form action="${contextPath}/qna/write.do" method="post" id="h1" enctype="multipart/form-data">
	 <hr><h1>글쓰기</h1><hr>
	 <table align="center">
	 			<tr>
					<td>상품 번호</td>
					<c:if test="${product_num == null}">
						<td>일반</td>
					</c:if>
					<c:if test="${product_num != null}">
						<td>${product_num}
				...
				<tr>
					<td>글제목</td>
					<td colspan="3"><input type="text" size="67"  maxlength="500" name="qna_title" class="input_box"/></td>
				</tr>
				<tr>
					<td>비밀번호</td>
					<td colspan="3"><input type="password" maxlength="10" id="qna_pw" name="qna_pw" class="input_box" /></td>
				</tr>	
				<tr>
					<td style="margin-top:5px;">글내용</td>
					<td colspan="3">
						<textarea class="form-control" id="q_content" name="qna_content"></textarea>
						<script type="text/javascript">
							CKEDITOR.replace('q_content', {height: 500});
						</script>
					</td>	
				</tr>		
				<tr>
					<td style="margin-top:5px;">이미지첨부 &nbsp;</td>
					<td><input type="file"  name="qna_img"  onchange="readURL(this);" /></td>
					<td id="view"></td>
				</tr> 		 		 		 
	 </table>
	 			<div align="center" id="write_button">
	 				<c:if test="${product_num == null}">
	 					<input type="button" value="글쓰기" onclick="fn_pw(this.form)">
	 				</c:if>
	 				<c:if test="${product_num != null}">
	 					<input type="button" value="글쓰기" onclick="fn_pw_p(this.form)">
	 				</c:if>
