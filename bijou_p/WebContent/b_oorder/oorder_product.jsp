...
function removeChar(event) {
	...
	function fn_point() {
		...
		var input_point = document.getElementById("input_point").value;

		if(member_point < input_point) {
			alert('포인트가 부족합니다.');
			return;
		} else {
			var member_point_n = member_point - input_point;
			document.getElementById("member_point_n").setAttribute("value", member_point_n);
			var oorder_price = document.getElementById("oorder_price").value - input_point;
	  ...
	  var form = document.fr; // 폼이름
		...
		if(form.oorder_name.value == "") {
			...
			return ;
				...
				form.action = "${contextPath}/oorder/product.do"; //다음페이지 주소
				form.submit(); // 폼 실행
		}
}
...
<hr><h1 align="center">주문하기</h1><hr>
<form action="${contextPath}/oorder/product.do" method="post" name="fr">
	<table class="table_1">
	<tr>
		<th><a href="${contextPath}/product/content.do?product_num=${pro_vo.product_num}">
			<img src="${contextPath}/download_p.do?product_num=${pro_vo.product_num}&product_img=${pro_vo.product_img}" alt="" width="200px"/></a>
		</th>
		<th><a href="${contextPath}/product/content.do?product_num=${pro_vo.product_num}">${pro_vo.product_name}</a></th>
		<th>주문수량 : ${oorder_count}</th>
			<td>결제 금액 : ${oorder_price}원</td>	
	</tr>			
	<tr>
	<th>주문자 정보</th>
	...
	<tr>
		<th>배송지 정보</th>
		<td><input type="checkbox" onclick="fn_check();" name="check"/> 주문자 정보 동일<br>
			<input type="text" value="" id="oorder_name" name="oorder_name" />
			<input type="text" value="" id="oorder_phone" name="oorder_phone" />
			...
			<input type="hidden" value="${pro_vo.product_num}" name="product_num" />
	...
	<tr>
		<th>포인트</th>
		<td>사용가능 포인트 : <input type="text" value="${mem_vo.member_point}" id="member_point" name="member_point" readonly/></td>
		<td>사용할 포인트 : <input type="text" placeholder="숫자만 입력" value="0" id="input_point" name="input_point" onblur="fn_point();" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)'/></td>
		<td>총 결제 금액 : <input type="text" value="${oorder_price}" name="oorder_price" id="oorder_price" readonly /></td>
